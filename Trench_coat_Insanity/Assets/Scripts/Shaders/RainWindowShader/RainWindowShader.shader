    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                drops += Layer(i.uv * 1.14 + 3.52, t);
                drops += Layer(i.uv * 1.82 + 7.17, t);
                drops += Layer(i.uv * 1.14 - 5.63, t);
                ...
                blur *= .1;
                float a = PseudoRandom(i.uv) * 6.2831;
                for (float i = 0; i < numSamples; i++)
                {
                    float2 offs = float2(sin(a), cos(a)) * blur;
                    float d = frac(sin((i+1) * 523) * 5273);
                    d = sqrt(d);
                    offs *= d;
